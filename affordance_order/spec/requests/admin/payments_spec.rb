...
RSpec.describe "Admin::Payments", type: :request do
  describe "GET /admin/payments" do
    ...
    context "with authentication" do
      ...
      it "filters by status" do
        order = create(:order)
        charged_payment = create(:payment, :charged, order: order)
        pending_payment = create(:payment, order: create(:order))
      ...
      it "shows payment totals" do
        create_list(:payment, 2, order: create(:order))
        get "/admin/payments", headers: admin_auth_headers
      ...
      it "shows fee amounts" do
        order = create(:order)
        payment = create(:payment, order: order, fee_cents: 50_000)
        get "/admin/payments", headers: admin_auth_headers
  ...
  describe "GET /admin/payments/:id" do
    context "without authentication" do
      it "returns 401" do
        payment = create(:payment, order: create(:order))
    ...
    context "with authentication" do
      ...
      it "shows currency" do
        order = create(:order)
        payment = create(:payment, order: order, currency: "RUB")
        get "/admin/payments/#{payment.id}", headers: admin_auth_headers
