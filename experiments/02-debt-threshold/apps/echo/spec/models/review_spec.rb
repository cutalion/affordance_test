...
RSpec.describe Review, type: :model do
  ...
  describe "validations" do
    ...
    it "validates request must be completed" do
      request = create(:request) # pending state
      client = request.client
      review = build(:review, request: request, author: client)
      expect(review).not_to be_valid
      expect(review.errors[:request]).to be_present
    end

    it "is valid when request is completed" do
      review = build(:review)
      expect(review).to be_valid
    end

    describe "uniqueness per request and author" do
      let(:request) { create(:request, :completed) }
      let(:client) { request.client }

      it "prevents duplicate review from same author on same request" do
        create(:review, request: request, author: client)
        duplicate = build(:review, request: request, author: client)
      ...
      it "allows different authors to review the same request" do
        ...
        second_review = build(:review, request: request, author: provider)
